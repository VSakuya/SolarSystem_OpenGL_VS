		...
		Star* parentStar)
{
	...
	this->EquatorialRotationV = this->OrbitalSpeed = 0;
	...
	this->speed = speed == 0 ? 0 : speed / (2 * PI * distance) * 360;
}
...
void Star::DrawStar()
{
	...
	int drawLines = 2 * int(radius) * 3 * 500;
	...
	glPushMatrix();
	{
		...
		{
			//将绘制的图形沿 z 轴旋转 alpha
			glRotatef(parentStar->OrbitalSpeed, 0, 0, 1);
		...
		glEnd();
		glRotatef(OrbitalSpeed, 0, 0, 1);
		glTranslatef(distance, 0, 0);

		glRotatef(EquatorialRotationV, 0, 0, 1);
...
{
 	OrbitalSpeed += deltaTime * speed; // 更新角度
	if (OrbitalSpeed >= 360)
	{
		OrbitalSpeed = 0;
	}
 	EquatorialRotationV += selfSpeed; // 更新自转角度
	if (EquatorialRotationV >= 360)
	{
		EquatorialRotationV = 0;
	}
}
